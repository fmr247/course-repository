...
$$\omega_\text{eff} = a \cdot \omega_\text{efp} + (1-a) \cdot \omega_\text{mvp}$$
...
$$
 ...
 where $SCO_j$ is the number of shares outstanding of stock $j$.
...
- [Discuss:]{style="color: red;"} What are the questionable assumptions behind the baseline regression framework?
...
1. Fix sample period $T$, estimation window size $h$, and out-of-sample horizon $K = T - h$
1. Respect the information constraint: never use data an investor would not have at the time of decision
...
![Rolling estimation window of size $h$; portfolio weights are recomputed each period and performance is measured out-of-sample.](figures/backtestoos.png)
...
#| echo: false
rolling_window <- 60
returns_matrix <- returns_monthly |> pivot_wider(names_from = symbol, values_from = ret) |> dplyr::select(-date)
weight_history <- map(seq(rolling_window + 1, nrow(returns_matrix)), function(t) {
  window <- returns_matrix[(t-rolling_window):(t-1),] 
  if (nrow(window) < 30) return(NULL)
  mu_w  <- colMeans(window)
  sig_w <- cov(window)
  w_raw <- tryCatch(solve(sig_w) %*% mu_w, error = function(e) NULL)
  if (is.null(w_raw) || abs(sum(w_raw)) < 1e-8) return(NULL)
  tibble(symbol = names(mu_w), 
         weight = as.vector(w_raw / sum(w_raw)),
         date = t)
}) |> compact() |> bind_rows()

weight_history |>
  ggplot(aes(x = date, y = weight, group = symbol)) +
  geom_line(alpha = 0.5) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  scale_y_continuous(labels = percent) +
  labs(x = NULL, y = "Portfolio weight",
       title = "Rolling tangency portfolio weights (60-month window)",
       subtitle = "Each line is one Dow Jones stock; no short-sell constraint")
```
...
set.seed(2026)
n_bootstrap <- 500
T_obs <- nrow(returns_matrix)

bootstrap_frontiers <- map(seq_len(n_bootstrap), function(b) {
  idx   <- sample(T_obs, T_obs, replace = TRUE)
  mu_b  <- colMeans(returns_matrix[idx, ])
  sig_b <- cov(returns_matrix[idx, ])
  sig_inv_b <- tryCatch(solve(sig_b), error = function(e) NULL)
  if (is.null(sig_inv_b)) return(NULL)
  iota_b <- rep(1, ncol(sig_b))
  mvp_b  <- (sig_inv_b %*% iota_b) / as.numeric(t(iota_b) %*% sig_inv_b %*% iota_b)
  C_b    <- as.numeric(t(iota_b) %*% sig_inv_b %*% iota_b)
  D_b    <- as.numeric(t(iota_b) %*% sig_inv_b %*% mu_b)
  E_b    <- as.numeric(t(mu_b)   %*% sig_inv_b %*% mu_b)
  lam_b  <- 2 * (max(mu_b) - D_b / C_b) / (E_b - D_b^2 / C_b)
  efp_b  <- as.vector(mvp_b + lam_b / 2 * (sig_inv_b %*% mu_b - D_b * mvp_b))
  if(t(mvp_b)%*%mu_b>=t(efp_b)%*%mu_b) return(NULL)
  tibble(a = seq(0, 1, by = 0.05), boot = b) |>
    mutate(
      omega = map(a, \(x) x * efp_b + (1 - x) * as.vector(mvp_b)),
      mu    = map_dbl(omega, \(x) t(x) %*% assets$mu),
      sigma = map_dbl(omega, \(x) sqrt(t(x) %*% sigma%*% x))
    )
}) |> bind_rows()

ggplot(bootstrap_frontiers, aes(x = sigma, y = mu, group = boot)) +
  geom_line(alpha = 0.15, color = "grey60") +
  geom_line(data = efficient_frontier, aes(group = NULL),
            color = "steelblue", linewidth = 1) +
  scale_x_continuous(labels = percent) +
  scale_y_continuous(labels = percent) +
  labs(x = "Volatility", y = "Expected return",
       title = "Bootstrap uncertainty of the efficient frontier",
       subtitle = "Grey: bootstrap frontiers (evaluated at full-sample parameters) | Blue: in-sample frontier")
```
...
mu_0      <- assets$mu
sig_0     <- sigma
sr_oracle <- sqrt(as.numeric(t(mu_0) %*% solve(sig_0) %*% mu_0))

sim_sr <- map_dbl(seq_len(n_sim), function(s) {
  draws <- mvrnorm(T_sim, mu = mu_0, Sigma = sig_0)
  mu_h  <- colMeans(draws)
  sig_h <- cov(draws)
  w_raw <- tryCatch(solve(sig_h) %*% mu_h, error = function(e) NULL)
  if (is.null(w_raw) || abs(sum(w_raw)) < 1e-8) return(NA_real_)
  w    <- w_raw / sum(w_raw)
  mu_p <- as.numeric(t(w) %*% mu_0)
  sd_p <- sqrt(as.numeric(t(w) %*% sig_0 %*% w))
  if(mu_p<0) return(NA)
  return(mu_p / sd_p)
})

tibble(sr = sim_sr) |> drop_na() |>
  ggplot(aes(x = sr)) +
  geom_histogram(fill = "steelblue", alpha = 0.8) +
  geom_vline(xintercept = sr_oracle, color = "red", linewidth = 1.2, linetype = "dashed") +
  annotate("text", x = sr_oracle, y = Inf, vjust = 2, hjust = 1.3,
           label = sprintf("Oracle SR = %.2f", sr_oracle), color = "red") +
  labs(x = "Out-of-sample Sharpe ratio", y = "Count",
       title = sprintf("Plug-in tangency SR distribution (N = %d, T = %d months)",
                       length(mu_0), T_sim),
       subtitle = "Red line: oracle Sharpe ratio under true parameters")
```
...
## plug-in vs. $1/N$ vs. constrained

- Rolling-window backtest with estimation window $h = 60$ months over the full Dow Jones sample
- For each period $t$, compute three sets of weights based on information up to $t$:
  1. **Plug-in tangency**: unconstrained $\hat\omega^\text{tan}(\hat\mu_t, \hat\Sigma_t)$
  2. **Naive**: equal-weight $\omega_\text{naive} = \frac{1}{N}\iota$
  3. **Constrained**: short-sell constrained portfolio ($\omega_i \geq 0$) via quadratic programming
- **Expected finding**: naive $1/N$ often beats unconstrained plug-in out-of-sample (DeMiguel, Garlappi, and Uppal, 2009) 

```{r}
#| echo: false
library(quadprog)
ret_wide <- returns_monthly |>
  pivot_wider(names_from = symbol, values_from = ret) |> arrange(date)
h_bt <- 60; gamma_bt <- 4
N_bt <- ncol(ret_wide) - 1; T_bt <- nrow(ret_wide)

backtest <- map(seq(h_bt + 1, T_bt - 1), function(t) {
  window <- ret_wide[(t - h_bt):(t - 1), -1] |> as.matrix()
  r_next <- as.numeric(ret_wide[t + 1, -1])
  mu_h   <- colMeans(window, na.rm = TRUE)
  sig_h  <- cov(window)
  iota_h <- rep(1, N_bt)
  w_naive  <- rep(1 / N_bt, N_bt)
  w_raw    <- tryCatch(solve(sig_h) %*% mu_h, error = function(e) NULL)
  w_tan_bt <- if (!is.null(w_raw) && abs(sum(w_raw)) > 1e-8)
    as.vector(w_raw / sum(w_raw)) else w_naive
  Dmat  <- sig_h + diag(1e-8, N_bt)
  Amat  <- cbind(iota_h, diag(N_bt))
  w_con <- tryCatch(
    solve.QP(Dmat, mu_h / gamma_bt, Amat, c(1, rep(0, N_bt)), meq = 1)$solution,
    error = function(e) w_naive
  )
  tibble(naive       = sum(w_naive   * r_next),
         tangency    = sum(w_tan_bt  * r_next),
         constrained = sum(w_con     * r_next))
}) |> bind_rows()
...
backtest |>
  ...
  pivot_longer(everything(), names_to = c("metric", "strategy"), names_sep = "_") |>
  pivot_wider(names_from = strategy, values_from = value) |>
  gt() |> fmt_number(columns = -metric, decimals = 2) |>
  tab_header(title = "Out-of-sample performance comparison (h = 60 months)")
```

## Shrinkage: continuous regularization

- Recall: the no-short solution implicitly uses $\tilde\Sigma = \Sigma - (\lambda\iota' + \iota\lambda')$
- Shrinkage estimators achieve similar regularization continuously, without hard constraints
- Ledoit and Wolf (2004): shrink $\hat\Sigma$ toward a structured target $F$
$$
\hat\Sigma_\delta = \delta F + (1-\delta)\hat\Sigma, \quad \delta \in [0,1]
$$

  - $F$: structured target (e.g., scaled identity, single-factor model, constant-correlation matrix)
  - $\delta$: optimal shrinkage intensity chosen analytically or with cross-validation
  - Better-conditioned $\hat\Sigma_\delta$ $\Rightarrow$ more stable weights $\Rightarrow$ lower out-of-sample losses