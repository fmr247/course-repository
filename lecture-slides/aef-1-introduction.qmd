...
- The tidyverse is "a framework [...] that aims at doing the cleaning and preparing steps much easier"
- [Wickhams and Grolemunds book *R for Data Science*](https://r4ds.hadley.nz/) explains everything we need
...
- Do not get confused. Some teaching ressources (and LLM) use (outdated) `%>%` instead of `|>` in R!
...
- Each variable forms a column. Each observation forms a row. Each cell is a single measurement
...
- `ggplot2`: *G*rammar of *g*raphics differentiates between the data and the representation: i) Data (data frame being plotted), ii) Geometrics (geometric shape that represents the data, e.g. point, boxplot, histogram), iii) Aesthetics (color, size, shape)
